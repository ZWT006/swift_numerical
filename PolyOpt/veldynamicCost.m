function [cost,grad] = veldynamicCost(coeffs,segpoly)
...
%     grad(((idi-1)*dim+2)*n_order + 1:((idi-1)*dim+3)*n_order) = qvgrad;% + qagrad;
    grad((idi-1)*dim*n_order + 1:((idi-1)*dim+1)*n_order)     = xvgrad;
    grad(((idi-1)*dim+1)*n_order + 1:((idi-1)*dim+2)*n_order) = yvgrad;
    grad(((idi-1)*dim+2)*n_order + 1:((idi-1)*dim+3)*n_order) = qvgrad;
    % gradt 直接置零
    if (TimeOptimal)
        gradt(idi) = vgradt;
    end
