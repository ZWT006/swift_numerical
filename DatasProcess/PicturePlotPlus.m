...
pt_ser = plot(PRMX_n*RATION,PRMY_n*RATION,'m--' , 'LineWidth',1);
plot(PRMX_n(1)*RATION,PRMY_n(1)*RATION,'d','Color','y','MarkerSize',10,'MarkerEdgeColor','y','MarkerFaceColor','y');
plot(PRMX_n(end)*RATION,PRMY_n(end)*RATION,'h','Color','g','MarkerSize',12,'MarkerEdgeColor','g','MarkerFaceColor','g');
...
topt = topt + Tv0;
tser=0:tstep:(PRM_k-1)*tstep;
ythold = max([abs(min(PRMY_dn)),abs(max(PRMY_dn)),abs(min(PRMX_dn)),abs(max(PRMX_dn)), ...
         abs(min(Y_dn)),abs(max(Y_dn)),abs(min(X_dn)),abs(max(X_dn)),pv_max]);

subx = subplot(2,1,1);
hold on
pt_xopt = plot(topt,X_dn, 'r-');
pt_xser = plot(tser,PRMX_dn, 'r--');
...
plot(tser(end),PRMX_dn(end),'^','Color','k','MarkerSize',6,'MarkerEdgeColor','k','MarkerFaceColor','k');
hold off
...
pt_yser = plot(tser,PRMY_dn, 'b--');
pt_boun = line([0,ceil(T)+2],[pv_max,pv_max],'linestyle','--','color','k','LineWidth',1);
line([0,ceil(T)+2],[-pv_max,-pv_max],'linestyle','--','color','k','LineWidth',1);
plot(topt(end),Y_dn(end),'p','Color','k','MarkerSize',8,'MarkerEdgeColor','k','MarkerFaceColor','k');
plot(tser(end),PRMY_dn(end),'^','Color','k','MarkerSize',6,'MarkerEdgeColor','k','MarkerFaceColor','k');
hold off
txt = ylabel('$v_y$[m/s]');
set(txt, 'Interpreter', 'latex');
...
pt_qser = plot(tser,rad2deg(PRMQ_n), 'm--');
plot(topt(end),rad2deg(Q_n(end)),'p','Color','k','MarkerSize',8,'MarkerEdgeColor','k','MarkerFaceColor','k');
plot(tser(end),rad2deg(PRMQ_n(end)),'^','Color','k','MarkerSize',6,'MarkerEdgeColor','k','MarkerFaceColor','k');
hold off
...
plot(tser(end),PRMQ_dn(end),'^','Color','k','MarkerSize',6,'MarkerEdgeColor','k','MarkerFaceColor','k');
hold off
txt = ylabel('$\omega_{\theta}$[rad/s]');
set(txt, 'Interpreter', 'latex');
ythold = max([abs(min(PRMQ_dn)),abs(max(PRMQ_dn)),abs(min(Q_dn)),abs(max(Q_dn)),wv_max]);
ylim([-ythold-1 ythold+1]);
...
plot(tser(end),PRMQ_ddn(end),'^','Color','k','MarkerSize',6,'MarkerEdgeColor','k','MarkerFaceColor','k');
hold off
txt = ylabel('$a_{\theta}$[rad/s${}^2$]'); %[rad/s^2]
set(txt, 'Interpreter', 'latex');
xlabel("time [s]");
xlim([0 ceil(T)+2]);
ythold = max([abs(min(PRMQ_ddn)),abs(max(PRMQ_ddn)),abs(min(Q_ddn)),abs(max(Q_ddn)),wa_max]);
...
topt = topt + Tv0;
tser=0:tstep:(PRM_k-1)*tstep;
ythold = max([abs(min(PRMY_ddn)),abs(max(PRMY_ddn)),abs(min(PRMX_ddn)),abs(max(PRMX_ddn)), ...
         abs(min(Y_ddn)),abs(max(Y_ddn)),abs(min(X_ddn)),abs(max(X_ddn)),pa_max]);

subx = subplot(2,1,1);
hold on
pt_xopt = plot(topt,X_ddn, 'r-');
pt_xser = plot(tser,PRMX_ddn, 'r--');
...
plot(tser(end),PRMX_ddn(end),'^','Color','k','MarkerSize',6,'MarkerEdgeColor','k','MarkerFaceColor','k');
hold off
...
pt_yser = plot(tser,PRMY_ddn, 'b--');
pt_boun = line([0,ceil(T)+2],[pa_max,pa_max],'linestyle','--','color','k','LineWidth',1);
line([0,ceil(T)+2],[-pa_max,-pa_max],'linestyle','--','color','k','LineWidth',1);
plot(topt(end),Y_ddn(end),'p','Color','k','MarkerSize',8,'MarkerEdgeColor','k','MarkerFaceColor','k');
plot(tser(end),PRMY_ddn(end),'^','Color','k','MarkerSize',6,'MarkerEdgeColor','k','MarkerFaceColor','k');
hold off
